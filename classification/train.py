...
if __name__ == '__main__':
    ...
    train_df, class_dict = construct_train_df(CFG.image_paths)
    ...
    for fold in range(CFG.n_fold):
        if fold in CFG.trn_fold:
            _oof_df = train_loop(train_df, class_dict, fold, device)
            oof_df = pd.concat([oof_df, _oof_df])

    _ = print_loss_and_metrics(oof_df['label'], oof_df['prediction'], fold=None, epoch=None, train_loss=None, val_loss=None)
